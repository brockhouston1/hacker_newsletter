...
def get_news_articles():
    ...
    news_article_ids = response.json()[:700]  # Limit to the top 10 for demonstration purposes
    count = 0
    ...
    print(f"| News Article |               {count}                 |")
    return data_news

def get_top_show_stories():
    data_shows = []
    response = requests.get('https://hacker-news.firebaseio.com/v0/showstories.json')
    show_story_ids = response.json()[:700]  # Limit to the top 10 for demonstration purposes
    count = 0
    ...
    print(f"| Show Stories |                {count}                 |")
    return data_shows

def get_job_postings():
    data_jobs = []
    response = requests.get('https://hacker-news.firebaseio.com/v0/jobstories.json')
    job_posting_ids = response.json()[:700]  # Limit to the top 10 for demonstration purposes
    count = 0
